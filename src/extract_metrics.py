# Functions to extract meaningful swing metrics
import cv2
from ultralytics import YOLO
import sys
import os
# sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from src.preprocess import preprocess_swing_video
import torch
import numpy as np
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms
from src.model import EventDetector
...
def Get_Metrics(input_path, YOLO_path, swingnet_path):
    pose_estimation = Get_Pose_Estimation(input_video_path=input_path, model_path=YOLO_path)
    print("Pose Estimation Complete")
    swing_timings = Get_Swing_Timings(input_path=input_path, model_path=swingnet_path)
    ...
    initial_com = swing_angles["Address"][-1]
    lean = {}
    for event in swing_angles.keys():
        lean[event] = swing_angles[event][-1] - initial_com
        # print(f"{event} lean: {lean[event]}")
    
    # Converts float32s to floats for FastAPI handling
    return {k: [round(float(x), 4) for x in v] for k, v in swing_angles.items()}, {k: round(float(v), 4) for k, v in lean.items()}


def Get_Pose_Estimation(input_video_path=None, model_path=None, *args, **kwargs):
    
    # Raise an error if critical arguments are missing
    if input_video_path is None or model_path is None:
        raise ValueError("input_video_path and model_path are required")
...
def get_swing_angles(keypoints, timings):
    ...
    address_angles = calculate_event_angles(keypoints[address][0])

    # Metrics at toe-up
    toe_up = timings["Toe-up"]["frame"]
    toe_up_angles = calculate_event_angles(keypoints[toe_up][0])

    # Metrics at mid-backswing
    mid_backswing = timings["Mid-backswing"]["frame"]
    mid_backswing_angles = calculate_event_angles(keypoints[mid_backswing][0])

    # Metrics at top
    top = timings["Top"]["frame"]
    top_angles = calculate_event_angles(keypoints[top][0])

    # Metrics at mid-downswing
    mid_downswing = timings["Mid-downswing"]["frame"]
    mid_downswing_angles = calculate_event_angles(keypoints[mid_downswing][0])

    # Metrics at impact
    impact = timings["Impact"]["frame"]
    impact_angles = calculate_event_angles(keypoints[impact][0])
...
def calculate_event_angles(keypoints):
    ...
    angles = [left_arm_shoulders, # 0
              right_arm_shoulders, # 1
              shoulders, # 2
              hips, # 3
              arm_angle_left, # 4
              arm_angle_right, # 5
...
def calculate_angle(p1, p2, p3):
    ...
    # Constant to avoid zero division
    epsilon = 1e-9
    # Compute angle in radians and convert to degrees
    angle_rad = np.arccos(dot_product / ((norm_v1+epsilon) * (norm_v2+epsilon)))
